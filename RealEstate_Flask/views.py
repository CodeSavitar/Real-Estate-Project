...
def addland_post():
    ...
    fh.close()
    if flag==0:
        dic={'Index':index,'Location':location,'Price':price,'Size':size,'Cost':cost,'Appreciation Value':app_val}
...
def addhome_post():
    ...
    fh.close()
    if flag==0:
        dic={'Index':index,'Number':number,'Name':name,'Location':location,'Price':price,'Bed':bed,'Bath':bath,'Appreciation Value':app_val}
        add=pd.DataFrame(dic,index=[num])
        df=df.append(add)
